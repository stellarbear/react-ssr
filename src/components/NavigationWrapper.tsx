...
const NavigationWrapper: React.FC<NavigationProps> =
	({ children, classes, history, rtl }): JSX.Element => {
		...
		const renderNavigationList = (handleChange: boolean): JSX.Element => (
			<React.Fragment>
				<Button onClick={(): void => handleNavigation('/home', handleChange)}>{t('navigation.home')}</Button>
				<Button onClick={(): void => handleNavigation('/about', handleChange)}>{t('navigation.about')}</Button>
